<section>
    ...
    <label for="<?php echo 'speech_post'.$clave; ?>" onclick="reproducir_contenido(<?php echo $clave; ?>)"><strong>Detalle de post<?php echo $clave+1; ?>📢</strong></label>
	...
	<p class="speech-post" style="display:none;" id="<?php echo "speech_post".$clave; ?>"></p>
            ...
            */
            ...
            <div id="<?php echo 'textos'.$clave; ?>">
                ...
                <p>
                 ...
                 echo ('Titulo '.$posts['titulo'].'Autor ' . $posts['autor'].'Fecha '.$posts['fecha']).'Categoria'.$posts['categoria']; ?>">
        ...
        <div name="mostrar_comentarios" style="display:none;">
            <?php $comentario = comentarios::mostrar($posts['CodPost']); ?>
                <?php 
                /* 
                    A diferencia de mg::mostrar que retornaba un conteo de los  me gusta aqui en cambio con comentarios::mostrar  aqui es en la seccion de "comentarios"(en la parte inferior de una publicacion),se mostrará "select U.nombre, C.comentario, C.CodCom from usuarios U inner join comentarios C on U.CodUsua = C.CodUsua where C.CodPost = :CodPost ORDER BY C.CodCom ASC")
                */ 
                ?>
